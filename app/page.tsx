...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("https://api.github.com/repos/kael-odin/cafe-ai/releases/latest")
      .then((res) => res.json())
      ...
      <div className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            />
            <span className="text-lg font-semibold tracking-tight">Cafe AI</span>
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <a
              href="https://github.com/kael-odin/cafe-ai"
              target="_blank"
              rel="noreferrer"
...
function HeroSection({ release }: { release: Release | null }) {
  const getDownloadUrl = () => {
    if (!release?.assets) return "https://github.com/kael-odin/cafe-ai/releases";
    return `https://github.com/kael-odin/cafe-ai/releases/tag/${release.tag_name}`;
  };
...
function DownloadSection({ release }: { release: Release | null }) {
  ...
  const getAssetSize = (pattern: string) => {
    if (!release?.assets) return null;
    const asset = release.assets.find((a) => a.name.includes(pattern));
      ...
      <div className="relative mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm backdrop-blur-sm mb-6">
            ...
            <span className="text-white/80">免费下载</span>
          </div>
          <h2 className="font-serif text-4xl sm:text-5xl lg:text-6xl leading-tight tracking-tight">
            选择你的平台
          </h2>
          <p className="mt-6 text-lg text-white/50 max-w-2xl mx-auto">
            {release ? `最新版本 ${release.tag_name}，全平台支持` : "全平台支持，一次下载永久免费"}
          </p>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-4xl mx-auto">
          {downloads.map((item, i) => {
            const downloadUrl = getAssetUrl(item.pattern);
            const fileSize = getAssetSize(item.pattern);

            return (
              <a
                key={i}
                href={downloadUrl || `https://github.com/kael-odin/cafe-ai/releases`}
                target="_blank"
                rel="noreferrer"
                className="group relative rounded-2xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-sm transition-all hover:bg-white/[0.06] hover:border-white/20 text-center"
              >
                <div className={`mb-4 inline-flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br ${item.color} border ${item.borderColor}`}>
                  <item.icon className={`h-7 w-7 ${item.iconColor}`} />
                </div>
                <h3 className="text-lg font-semibold mb-1">{item.platform}</h3>
                <p className="text-sm text-white/50 mb-3">{item.description}</p>
                {fileSize && (
                  <span className="text-xs text-white/30">{fileSize}</span>
                )}
                <div className="mt-4 inline-flex items-center gap-2 text-sm text-amber-400 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="mt-12 text-center">
          <a
            href="https://github.com/kael-odin/cafe-ai/releases"
            target="_blank"
            rel="noreferrer"
      ...
      <div className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <a
              href="https://github.com/kael-odin/cafe-ai/blob/main/README.md"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white transition-colors"
            >
